...
/** Per-cascade history of sent message texts (for user turn matching) */
export const sentMessageHistory = new Map();
...
async function _updateSnapshots() {
  ...
  for (const [id, c] of entries) {
    ...
    try {
      const snap = await captureHTML(c.cdp, { sentHistory: sentMessageHistory.get(id) || [] });
...
export async function refreshSnapshotOnce(cascadeId) {
  ...
  try {
    const snap = await captureHTML(c.cdp, { sentHistory: sentMessageHistory.get(cascadeId) || [] });
