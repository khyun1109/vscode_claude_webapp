...
export async function captureHTML(cdp, options = {}) {
  ...
  const sentHistory = options.sentHistory || [];
  const SCRIPT = `(() => {
    const keepInputs = ${keepInputs ? 'true' : 'false'};
    const sentHistory = ${JSON.stringify(sentHistory)};
    const isWorkbench = !!document.querySelector('.monaco-workbench, #workbench')
      || (document.body && document.body.className && document.body.className.includes('monaco-workbench'));
    ...
    if (origChildren.length > 0) {
      ...
      const userKeywords = ['user', 'human'];
      const assistantKeywords = ['assistant', 'ai', 'bot', 'claude', 'model'];
      ...
      // --- Strategy 0: Sent message text matching ---
      // Uses the history of messages the user actually sent through the webapp.
      // Normalizes whitespace for fuzzy comparison. This is the most reliable
      // strategy because we know the exact text the user typed.
      let detected = false;
      let usedStrategy = 'none';
      if (sentHistory.length > 0) {
        const normWs = (s) => (s || '').replace(/\\s+/g, ' ').trim().toLowerCase();
        const sentNormalized = sentHistory.map(normWs);

        for (const child of origChildren) {
          const childText = normWs(child.innerText || child.textContent || '');
          if (childText.length < 1) continue;
          const isMatch = sentNormalized.some(sent => {
            if (sent.length < 1) return false;
            // Exact match or the child text starts/ends with the sent text
            // (the UI may prepend/append decorations like "You" label)
            if (childText === sent) return true;
            if (sent.length >= 10 && childText.includes(sent)) return true;
            // For short messages, require tighter match (sent text is the core)
            if (sent.length >= 3 && sent.length < 10 && childText.startsWith(sent)) return true;
            return false;
          });
          if (isMatch) {
            markUser(child);
        ...
        if (detected) usedStrategy = 'strategy0-sent-text-match';
      }

      // --- Strategy 1: Explicit data attributes ---
      if (!detected) {
        for (const child of origChildren) {
          const role = lc(child.getAttribute('data-role'));
          const testId = lc(child.getAttribute('data-testid'));
          const ariaLabel = lc(child.getAttribute('aria-label'));
          const combined = role + ' ' + testId + ' ' + ariaLabel;
          if (userKeywords.some(k => combined.includes(k))) {
            markUser(child);
            detected = true;
            usedStrategy = 'strategy1-data-attrs';
          }
        }
      }

      // --- Strategy 1.5: VS Code Claude extension class patterns ---
      // The VS Code Claude extension uses minified class names with known patterns:
      // User turns: .P.X (without .e)  |  Assistant turns: .P.e
      // These appear as children of a .Y container.
      if (!detected) {
        const yContainer = origTurns.classList && origTurns.classList.contains('Y')
          ? origTurns
          : origTurns.querySelector('.Y');
        const searchRoot = yContainer || origTurns;
        const candidates = yContainer
          ? Array.from(searchRoot.children).filter(c => c.tagName === 'DIV')
          : origChildren;
        ...
        if (vsCodeDetected) {
          detected = true;
          usedStrategy = 'strategy1.5-vscode-class-pattern';
        }
      }

      // --- Strategy 2: Class name keyword matching ---
      if (!detected) {
        for (const child of origChildren) {
          const cls = lc(child.className);
          if (userKeywords.some(k => cls.includes(k)) &&
              !assistantKeywords.some(k => cls.includes(k))) {
            markUser(child);
            detected = true;
            usedStrategy = 'strategy2-class-keywords';
          }
        }
      }

      // --- Strategy 3: Computed style analysis (alignment/background) ---
      if (!detected) {
        const styleInfos = origChildren.map(child => {
        ...
        const rightAligned = styleInfos.filter(s =>
          s.alignSelf === 'flex-end' || s.alignSelf === 'end' ||
          s.justifyContent === 'flex-end' ||
          (s.marginLeft === 'auto' && s.marginRight !== 'auto')
        );
        if (rightAligned.length > 0 && rightAligned.length < origChildren.length) {
          rightAligned.forEach(s => markUser(s.el));
          detected = true;
          usedStrategy = 'strategy3-alignment';
        }
        ...
        if (!detected) {
          ...
          styleInfos.forEach(s => {
            const key = s.bg;
            if (!bgGroups.has(key)) bgGroups.set(key, []);
            bgGroups.get(key).push(s);
          });
          if (bgGroups.size === 2) {
            const groups = Array.from(bgGroups.values());
            const firstBg = styleInfos[0].bg;
            const userGroup = groups.find(g => g[0].bg === firstBg) || groups[0];
            const otherGroup = groups.find(g => g !== userGroup);
            if (userGroup.length <= otherGroup.length) {
              userGroup.forEach(s => markUser(s.el));
              detected = true;
              usedStrategy = 'strategy3-bg-groups';
            }
          }
        }
      }

      // --- Strategy 4: Class token frequency analysis ---
      if (!detected) {
        const tokenSets = origChildren.map(child => {
        ...
        if (origChildren.length > 2) {
          ...
          const tokenChildren = new Map();
          tokenSets.forEach((tset, idx) => {
            for (const t of tset) {
              tokenCounts.set(t, (tokenCounts.get(t) || 0) + 1);
              if (!tokenChildren.has(t)) tokenChildren.set(t, []);
              tokenChildren.get(t).push(origChildren[idx]);
            }
          });

          const total = origChildren.length;

          let assistToken = null;
          let assistCount = 0;
          for (const [t, count] of tokenCounts) {
            if (count > total * 0.5 && count < total && count > assistCount) {
              assistCount = count;
              assistToken = t;
            }
          }
          ...
          if (assistToken) {
            for (const [t, count] of tokenCounts) {
              if (count > 1 && count < total * 0.4 && count < bestCount) {
                const elems = tokenChildren.get(t);
                const hasAssist = elems.some(el => {
                  const cls = (el.className || '').toString().trim();
                  return cls.split(/\\s+/).includes(assistToken);
                });
                if (!hasAssist) {
                  bestCount = count;
                  bestToken = t;
          ...
          if (!bestToken) {
            for (const [t, count] of tokenCounts) {
              if (count > 1 && count < total * 0.25 && count < bestCount) {
                bestCount = count;
                bestToken = t;
              }
            }
          ...
          if (bestToken) {
            tokenChildren.get(bestToken).forEach(el => markUser(el));
            detected = markedElements.length > 0;
            if (detected) usedStrategy = 'strategy4-token-frequency';
          }
        }
      }

      // --- Strategy 4b: Two exact class groups ---
      if (!detected) {
        const classGroups = new Map();
        origChildren.forEach(child => {
          const cls = (child.className || '').toString().trim();
          if (!classGroups.has(cls)) classGroups.set(cls, []);
          classGroups.get(cls).push(child);
        });

        if (classGroups.size === 2) {
          const firstClass = (origChildren[0].className || '').toString().trim();
          origChildren.forEach(turn => {
            const cls = (turn.className || '').toString().trim();
            if (cls === firstClass) markUser(turn);
          });
          detected = markedElements.length > 0;
          if (detected) usedStrategy = 'strategy4b-class-groups';
        }
      }

      // --- Strategy 5: Alternating pattern ---
      if (!detected) {
        origChildren.forEach((turn, i) => {
          if (i % 2 === 0) markUser(turn);
        });
        usedStrategy = 'strategy5-alternating';
      }

      console.log('[captureHTML] User turn detection: strategy=' + usedStrategy +
        ', children=' + origChildren.length + ', marked=' + markedElements.length);
    }
...
/**
 * Diagnostic: Capture the DOM structure of the chat area to debug
 * user turn detection. Returns detailed info about each child element
 * in the turns container and what each strategy would detect.
 */
export async function diagnoseTurnDetection(cdp, sentHistory = []) {
  const SCRIPT = `(() => {
    const sentHistory = ${JSON.stringify(sentHistory)};
    const selectors = ${JSON.stringify(ROOT_SELECTORS)};
    ...
    const findTurnsContainer = (startEl) => {
      ...
      for (let d = 0; d < 15; d++) {
        ...
        path.push({ depth: d, childCount: kids.length, bestLen, totalLen, descended: kids.length <= 2 || (bestLen > totalLen * 0.7) });
        if (kids.length <= 2 || (bestLen > totalLen * 0.7)) {
          cur = best;
        } else {
          break;
    ...
    const turnsInfo = findTurnsContainer(root);
    const origTurns = turnsInfo.el;
    const origChildren = Array.from(origTurns.children).filter(c => c.tagName === 'DIV');

    const lc = (s) => (s || '').toString().toLowerCase();

    // Collect detailed info about each child
    const childrenInfo = origChildren.map((child, idx) => {
      const cls = (child.className || '').toString().trim();
      const tokens = cls.split(/\\s+/).filter(Boolean);
      ...
      return {
        ...
        classTokens: tokens,
        id: child.id || null,
        dataRole: child.getAttribute('data-role'),
        dataTestId: child.getAttribute('data-testid'),
        ariaLabel: child.getAttribute('aria-label'),
        textPreview: text.substring(0, 120) + (text.length > 120 ? '...' : ''),
        ...
        hasYClass: child.classList.contains('Y'),
        hasPClass: tokens.includes('P'),
        hasXClass: tokens.includes('X'),
        hasEClass: tokens.includes('e'),
      };
    });

    // Check for .Y container
    const yContainer = origTurns.classList && origTurns.classList.contains('Y')
      ? origTurns
      : origTurns.querySelector('.Y');
    let yChildrenInfo = null;
    if (yContainer && yContainer !== origTurns) {
      const yKids = Array.from(yContainer.children).filter(c => c.tagName === 'DIV');
      yChildrenInfo = yKids.map((child, idx) => {
        const cls = (child.className || '').toString().trim();
        const tokens = cls.split(/\\s+/).filter(Boolean);
        const text = (child.innerText || child.textContent || '').trim();
        return {
    ...
    const normWs = (s) => (s || '').replace(/\\s+/g, ' ').trim().toLowerCase();
    const sentNormalized = sentHistory.map(normWs);
    const strategy0Matches = origChildren.map((child, idx) => {
      const childText = normWs(child.innerText || child.textContent || '');
      const matched = sentNormalized.filter(sent => {
        if (sent.length < 1) return false;
        if (childText === sent) return true;
        if (sent.length >= 10 && childText.includes(sent)) return true;
        if (sent.length >= 3 && sent.length < 10 && childText.startsWith(sent)) return true;
      ...
      return { index: idx, matched: matched.length > 0, matchedTexts: matched };
    }).filter(m => m.matched);

    return {
      rootSelector: rootInfo.selector,
      turnsContainerPath: turnsInfo.path,
      turnsContainerClass: (origTurns.className || '').toString().trim(),
      turnsContainerTag: origTurns.tagName,
      childCount: origChildren.length,
      children: childrenInfo,
      yContainerFound: !!yContainer,
      yContainerIsRoot: yContainer === origTurns,
      yChildren: yChildrenInfo,
      sentHistoryCount: sentHistory.length,
      strategy0Matches,
      bodyClasses: (document.body?.className || '').toString().trim().substring(0, 200),
    };
  })()`;
